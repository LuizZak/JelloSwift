...
public struct InternalSpring: Codable {
    ...
    /// automatically whenever `initialRestDistanceSquared` is set.
    ...
    public mutating func updatePlasticity(distance: JFloat) {
        guard let plasticity = plasticity else {
            return
        }

        restDistance =
            calculatePlasticity(
                distance: distance, restDistance: restDistance,
