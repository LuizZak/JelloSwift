...
public struct ClosedShape: Codable, Equatable, ExpressibleByArrayLiteral {
    ...
    /// Re-centers the points of this closed shape so its centroid (geometric
    /// center) lays on (0, 0).
    @inlinable
...
extension ClosedShape {
    ...
    @inlinable
    public mutating func transformOwnBy(
    ...
    /// position.
    @inlinable
    public func transformedBy(
        translatingBy worldPos: Vector2 = .zero,
        rotatingBy angleInRadians: JFloat = 0,
        scalingBy localScale: Vector2 = .unit
    ) -> ClosedShape {
